...
"local")||$elt,target=$elt.data("target");$(local).each(function(){self._updatePreview($(this),color,type,cssObject)}),$input.val(color),color=$elt.data("important")?color+" !important":color;var name=$input.attr("name");cssObject[name]={selector:target,value:color}}),self._initializeColorPicker($this,$input)}),$popup.show()})},_initUploader:function(){var $uploadContainer=$("#upload-stylesheet");$uploadContainer.on("upload.uploader",function(e,file,interactionHook){interactionHook.error?feedback().error(interactionHook.error):feedback().success(interactionHook.success)}),$uploadContainer.on("fail.uploader",function(e,file,interactionHook){feedback().error(
interactionHook.message)}),$uploadContainer.uploader({uploadUrl:$uploadContainer.data("url")})},_initializeColorPicker:function($container,input){var color=input.val();$.farbtastic($container).setColor(color)},_updatePreview:function($container,color,type,cssObject){var oldStyle=$container.attr("style"),newStyle=oldStyle;if(void 0===oldStyle)newStyle=type+":"+color+" !important";else if(-1===oldStyle.indexOf(type))newStyle=oldStyle+" "+type+":"+color+" !important";else{var res=new RegExp(type+":[^;]+").exec(oldStyle);newStyle=oldStyle.replace(res,type+":"+color+" !important;")}if($container.attr("style",newStyle),"color"===type&&$container.hasClass(
"action-bar")){var calculatedColor=$container.css(type).replace("rgb","rgba").replace(")",",.3)");$container.find(".btn-info").css("border-color",calculatedColor),cssObject["action-border-color"]={selector:$container.data("calculus"),value:calculatedColor}}if("background"===type&&$container.hasClass("active")){calculatedColor=$container.css(type).replace("rgb","rgba").replace(")",",.5)");$("<style>.main-menu li a:hover{background:"+calculatedColor+" !important;}</style>").appendTo("head"),cssObject["hover-background"]={selector:$container.data("calculus"),value:calculatedColor}}}}}),define("taoThemingPlatformloader\taoThemingPlatform.bundle",function(){});
